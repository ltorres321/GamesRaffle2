...
class SqlServerDatabase {
  ...
  async initialize() {
    try {
      logger.info('Initializing SQL Server database connection...');
      ...
      logger.info('SQL Server database connection established successfully');
      
      // Test the connection
      await this.testConnection();
      
    } catch (error) {
      logger.error('Failed to initialize SQL Server database connection:', error);
      throw error;
    }
  }
  ...
  async testConnection() {
    try {
      ...
      if (result.recordset[0].test === 1) {
        logger.info('SQL Server database connection test passed');
        return true;
      } else {
        throw new Error('SQL Server database connection test failed');
      }
    } catch (error) {
      logger.error('SQL Server database connection test failed:', error);
      throw error;
    }
  }
  ...
  async isHealthy() {
    try {
      ...
      logger.error('SQL Server database health check failed:', error);
      return false;
    }
  }
  ...
  async query(queryText, params = {}) {
    try {
      ...
      // Normalize result to match PostgreSQL format
      return {
        rows: result.recordset,
        rowCount: result.rowsAffected[0] || 0
      };
    } catch (error) {
      logger.error('SQL Server database query error:', { query: queryText, params, error: error.message });
      throw error;
    }
  }
  ...
  async execute(procedureName, params = {}) {
    try {
      ...
      logger.error('SQL Server database procedure execution error:', { procedure: procedureName, params, error: error.message });
      throw error;
    }
  }
  ...
  async transaction(callback) {
    ...
    try {
      ...
      logger.error('SQL Server database transaction error:', error);
      
      try {
        await transaction.rollback();
      } catch (rollbackError) {
        logger.error('SQL Server database transaction rollback error:', rollbackError);
      }
  ...
  async close() {
    try {
      ...
      logger.error('Error closing SQL Server database connection:', error);
      throw error;
    }
  }
